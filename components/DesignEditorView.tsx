    ...
    <div className={`flex flex-col h-full transition-all duration-500 relative overflow-hidden ${theme === 'dark' ? 'bg-black' : 'bg-zinc-50'}`}>
      ...
      <div className="flex-1 relative overflow-hidden flex items-center justify-center bg-[#0a0a0a]">
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setActiveElementId(null);
          }}
        >
          {/* Main Product Frame - Maximized Size */}
          <div className={`relative w-full h-full flex items-center justify-center transition-transform duration-700 ${designData.view === 'Belakang' ? 'scale-125' : 'scale-100'}`}>
            <img
                ...
                >
                  {el.type === 'text' ? (
                    ...
                    <div className={`w-36 h-36 flex items-center justify-center p-2 rounded-3xl border-2 transition-all ${activeElementId === el.id ? 'neon-border bg-emerald-500/20 ring-4 ring-emerald-500/10 shadow-premium' : 'border-transparent'}`}>
                      <img
                        src={el.content}
                        className="max-w-full max-h-full object-cover filter drop-shadow-2xl"
                        draggable={false}
          ...
          <div className="absolute top-8 left-8 bg-black/90 px-6 py-2 rounded-[24px] text-[8px] font-black neon-text uppercase tracking-[0.3em] border border-white/10 shadow-2xl pointer-events-none z-50">
      ...
      <div className={`p-8 border-t z-[110] shrink-0 ${theme === 'dark' ? 'bg-zinc-950 border-white/5' : 'bg-white border-zinc-100 shadow-2xl'}`}>
        ...
        {activeElementId && (
          <div className="mb-8 px-6 py-4 rounded-[28px] bg-black/30 border border-white/5 flex items-center gap-6 animate-fade-in shadow-inner">
            ...
            {/* Simple Crop / Shape Toggle for Images */}
            {elements.find(el => el.id === activeElementId)?.type === 'image' && (
              <button
                onClick={() => {
                  const el = elements.find(e => e.id === activeElementId);
                  if (el) updateElement(activeElementId!, { borderRadius: el.borderRadius ? undefined : '50%' });
                }}
                className="p-3 bg-zinc-800 rounded-full text-[9px] font-black uppercase text-white hover:bg-zinc-700"
                title="Toggle Circle Crop"
              >
                {elements.find(el => el.id === activeElementId)?.borderRadius ? 'KOTAK' : 'BULAT'}
              </button>
            )}
          </div>
        )}
